...
function vanillaSelectBox(domSelector, options) {
    ...
    if (options) {
        ...
        if (options.useReverse != undefined) {
            this.userOptions.useReverse = options.useReverse;
        }
    ...
    this.getCssArray = function (selector) {
        ...
        if (selector === ".vsb-main button") {
            cssArray = [
                ...
                { "key": "border", "value": "1px solid #ccc !important" },
                { "key": "line-height", "value": "26px" },
                { "key": "font-size", "value": "16px" },
                { "key": "padding", "value": "6px 20px 6px 3px" }
            ]
        }
        ...
        this.drop.addEventListener("click", function (e) {
            ...
            if (!self.isMultiple) {
                ...
                if (nrAll == nrActives - Number(!self.userOptions.disableSelectAll)) {
                    ...
                    if (nrActives > self.multipleSize) {
                        let wordForItems = nrActives === 1 ? self.userOptions.translations.item : self.userOptions.translations.items;
                        // if (deselected_items === 0) {
                        //     selectedTexts = "no items selected"
                        // }

                        if (self.userOptions.useReverse != true) {
                            let deselected_items = nrAll - nrActives
                            if (deselected_items === 1) {
                                selectedTexts = nrAll - nrActives + " " + "form deselected"
                            }
                            else {
                                selectedTexts = nrAll - nrActives + " " + "forms deselected"
                            }
                        }
                        else {
                            let selected_items = nrActives
                            if (selected_items === 1) {
                                selectedTexts = nrActives + " " + "form selected"
                            }
                            else {
                                selectedTexts = nrActives + " " + "forms selected"
                            }
                        }
                    }
                }
